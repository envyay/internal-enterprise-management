...
from domain.entities.schemas import QueryAnalysis
from .prompts import *
from shared_kernel.utils import estimate_context_tokens

if config.MAIN_HISTORY_MESSAGES_TO_KEEP < 2:
    raise ValueError("MAIN_HISTORY_MESSAGES_TO_KEEP must be at least 2.")

PRE_ANSWER_HISTORY_MESSAGES_TO_KEEP = max(config.MAIN_HISTORY_MESSAGES_TO_KEEP - 1, 0)
...
def _retrieval_contexts(messages) -> list[str]:
    ...
    for message in messages:
        ...
        if content and not content.startswith(ignored_prefixes):
            parts = content.split(config.CHILD_CHUNK_SEPARATOR) if message.name == "search_child_chunks" else [content]
